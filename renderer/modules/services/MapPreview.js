...
const STANDARD_SNAKING_SLIDERS = false;
const STANDARD_SLIDER_END_CIRCLES = true;
...
const state = {
    ...
    catchCatcherTrailSamples: [],
    catchHyperDashHitEffects: [],
    catchTriggeredHitEffects: new Set(),
    catchLastEffectTime: Number.NaN,
    catchRenderObjects: null,
    audio: new Audio(),
...
const getStandardFadeInMs = (preemptMs) => (
    STANDARD_FADE_IN_BASE_MS
    * Math.min(1, Math.max(0, preemptMs) / STANDARD_FADE_IN_PREEMPT_THRESHOLD_MS)
);

const calculateManiaScrollTimeMs = (scrollSpeed, bpm = 120, scaleWithBpm = false) => {
    const normalizedSpeed = clamp(Math.round(Number(scrollSpeed) || DEFAULT_MANIA_SCROLL_SPEED), 1, 40);
...
const applyPreviewStacking = (objects, approachRate, stackLeniency) => {
    ...
    for (const object of objects) {
        ...
        delete object._cachedStandardSliderTicks;
        delete object._cachedStandardSliderTickRate;
        delete object._cachedStandardSliderStackIndex;
    }
...
const getTaikoTimingState = (time) => {
    const timingPoints = Array.isArray(state.mapData?.timingPoints) ? state.mapData.timingPoints : [];
    ...
    for (const point of timingPoints) {
        ...
        if (point.uninherited && point.beatLength > 0) {
            ...
            svMultiplier = -100 / point.beatLength;
        }
    }
...
const getTaikoPixelsPerMs = (time, playfieldWidth) => {
    const timing = getTaikoTimingState(time);
    const baseSliderVelocity = Number.isFinite(state.mapData?.sliderMultiplier) && state.mapData.sliderMultiplier > 0
        ? state.mapData.sliderMultiplier
        : 1.4;
    const scale = playfieldWidth / OSU_PLAYFIELD_WIDTH;
    const pixelsPerBeat = 100 * baseSliderVelocity * timing.svMultiplier * scale;
    return pixelsPerBeat / Math.max(1, timing.beatLength);
};

const getTaikoMeasureLines = (visibleStart, visibleEnd) => {
    const controlPoints = (Array.isArray(state.mapData?.timingPoints) ? state.mapData.timingPoints : [])
        .filter((point) => point?.uninherited && point.beatLength > 0)
        .map((point) => ({
            time: point.time,
            beatLength: point.beatLength,
            meter: 4,
...
const renderTaikoMode = ({
    ...
    const currentTaikoSpeed = Math.max(getTaikoPixelsPerMs(currentTime, playfieldWidth), 0.001);
    ...
    const taikoPositionAt = (targetTime, speedReferenceTime = targetTime) => (
        judgeX + ((targetTime - currentTime) * getTaikoPixelsPerMs(speedReferenceTime, playfieldWidth))
    );
    ...
    for (const measureTime of getTaikoMeasureLines(measureVisibleStart, visibleEnd)) {
...
const getCatchTimingState = (time) => {
    const timingPoints = Array.isArray(state.mapData?.timingPoints) ? state.mapData.timingPoints : [];
    let beatLength = 60000 / 120;
...
const buildCatchSliderRenderObjects = (object) => {
    const renderObjects = [];
    const totalDuration = Math.max(1, (object.endTime || object.time) - object.time);
    const slides = Math.max(1, object.slides || 1);
    const spanDuration = totalDuration / slides;
    const beatLength = getCatchTimingState(object.time).beatLength;
    const sliderTickRate = Number.isFinite(state.mapData?.sliderTickRate) && state.mapData.sliderTickRate > 0
        ? state.mapData.sliderTickRate
        : 1;
    ...
    for (let spanIndex = 0; spanIndex < slides; spanIndex++) {
...
const buildCatchSpinnerRenderObjects = (object) => {
    const renderObjects = [];
    const duration = Math.max(1, object.endTime - object.time);
    const beatLength = getCatchTimingState(object.time).beatLength;
    ...
    for (let i = 0; i <= count; i++) {
        ...
        const x = deterministicUnitValue(seed) * OSU_PLAYFIELD_WIDTH;
        renderObjects.push({
            time,
            x,
            type: 'banana',
...
const getCatchRenderObjects = () => {
    if (Array.isArray(state.catchRenderObjects)) {
        return state.catchRenderObjects;
    }

    const built = [];
    const objects = Array.isArray(state.mapData?.objects) ? state.mapData.objects : [];
    for (const object of objects) {
        if (!object) {
            continue;
        }
        ...
        if (object.kind === 'spinner') {
            built.push(...buildCatchSpinnerRenderObjects(object));
        } else if (object.kind === 'slider') {
            built.push(...buildCatchSliderRenderObjects(object));
        } else {
            built.push({
                time: object.time,
                x: object.x,
    ...
    state.catchRenderObjects = built;
    return built;
};

const applyCatchHyperDashIndicators = (catchObjects, catcherWidthOsu) => {
...
const renderCatchMode = ({
    ...
    const catchObjects = getCatchRenderObjects();
    ...
    const catcherWidthOsu = (catcherWidth / Math.max(1, playfieldWidth)) * OSU_PLAYFIELD_WIDTH;
    ...
    const walkSpeedPxPerMs = (playfieldWidth / OSU_PLAYFIELD_WIDTH) * 0.5;
    ...
    const actionableCatchTargets = catchObjects.filter((object) => object && object.type !== 'banana');
    ...
    let nextTarget = null;
    if (actionableCatchTargets.length > 0) {
        for (const object of actionableCatchTargets) {
            if (object.time <= currentTime) {
    ...
    const defaultCatcherStartX = playfieldX + (playfieldWidth / 2);
    let catcherX = Number.isFinite(lastRenderX)
      ? lastRenderX
      : defaultCatcherStartX;
    if (previousTarget && nextTarget && nextTarget.time > previousTarget.time) {
      const previousX = mapX(previousTarget.x);
      const nextX = mapX(nextTarget.x);
      const moveSpeedPxPerMs = resolveCatchMoveSpeedPxPerMs(
        previousX,
        nextX,
        nextTarget.time - previousTarget.time,
        previousTarget.hyperDash
      );
    ...
    } else if (nextTarget && nextTarget.time > currentTime) {
      const nextX = mapX(nextTarget.x);
      const movementStartX = previousTarget ? mapX(previousTarget.x) : defaultCatcherStartX;
      const movementStartTime = previousTarget ? previousTarget.time : 0;
      const moveSpeedPxPerMs = resolveCatchMoveSpeedPxPerMs(
        movementStartX,
        nextX,
        nextTarget.time - movementStartTime,
        previousTarget?.hyperDash || false
      );
      catcherX = getCatchTravelPosition(
        movementStartX,
        nextX,
      ...
      if (Number.isFinite(lastRenderTime) && deltaTime > 0 && deltaTime <= 220 && Number.isFinite(lastRenderX)) {
        const maxStep = Math.max(walkSpeedPxPerMs, moveSpeedPxPerMs) * deltaTime * 1.25;
        const distancePx = catcherX - lastRenderX;
        if (Math.abs(distancePx) > maxStep) {
          catcherX = lastRenderX + (Math.sign(distancePx) * maxStep);
        }
      }
    } else if (previousTarget) {
      catcherX = mapX(previousTarget.x);
    }

    state.catcherRenderX = catcherX;
    if (!Number.isFinite(lastRenderX) || !Number.isFinite(lastRenderTime) || deltaTime < 0 || deltaTime > 220) {
        state.catchCatcherTrailSamples = [];
    }
    state.catcherRenderTime = currentTime;
    catcherX = state.catcherRenderX;

    if (!Number.isFinite(state.catchLastEffectTime) || currentTime < state.catchLastEffectTime - 8 || currentTime > state.catchLastEffectTime + 2000) {
        state.catchHyperDashHitEffects = [];
        state.catchTriggeredHitEffects = new Set();
    }
    state.catchLastEffectTime = currentTime;
    ...
    const catcherVelocity = (Number.isFinite(lastRenderX) && Number.isFinite(deltaTime) && deltaTime > 0)
        ...
        : 0;
    state.catchCatcherTrailSamples.push({ time: currentTime, x: catcherX });
    const trailWindowMs = 220;
    state.catchCatcherTrailSamples = state.catchCatcherTrailSamples.filter((sample) => (currentTime - sample.time) <= trailWindowMs);
    ...
    if (velocityAfterimageCount > 0 && state.catchCatcherTrailSamples.length > 1) {
        const trailLookbackMs = 36 + (170 * velocityTrailStrength);
        for (let i = velocityAfterimageCount; i >= 1; i--) {
            const targetAge = (i / velocityAfterimageCount) * trailLookbackMs;
            const targetTime = currentTime - targetAge;
            let trailSample = state.catchCatcherTrailSamples[0];
            for (const sample of state.catchCatcherTrailSamples) {
                trailSample = sample;
    ...
    for (const object of catchObjects) {
        ...
        if (!(object.hyperDash || object.hyperDashFollowUp) || state.catchTriggeredHitEffects.has(object.renderId)) {
            continue;
        }
        ...
        state.catchTriggeredHitEffects.add(object.renderId);
        state.catchHyperDashHitEffects.push({
            x: catcherX,
            y: catcherY,
    ...
    for (const effect of state.catchHyperDashHitEffects) {
    ...
    state.catchHyperDashHitEffects = remainingHitEffects;
    ...
    for (const object of catchObjects) {
        ...
        let color = comboColours[object.comboIndex % comboColours.length] || DEFAULT_COMBO_COLOURS[0];
...
const getManiaReferenceBpm = () => {
    const fallbackBpm = Number(state.mapData?.bpmMin);
    return Number.isFinite(fallbackBpm) && fallbackBpm > 0 ? fallbackBpm : 120;
};

const getManiaTimingState = (time) => {
    const timingPoints = Array.isArray(state.mapData?.timingPoints) ? state.mapData.timingPoints : [];
    let beatLength = 60000 / 120;
    let svMultiplier = 1;
...
const getManiaPixelsPerMs = (time, playfieldHeight) => {
    const timing = getManiaTimingState(time);
    const baseScrollTimeMs = calculateManiaScrollTimeMs(DEFAULT_MANIA_SCROLL_SPEED, 120, false);
    const basePixelsPerMs = playfieldHeight / Math.max(1, baseScrollTimeMs);
    const referenceBpm = DEFAULT_MANIA_SCROLL_SCALE_WITH_BPM ? 120 : getManiaReferenceBpm();
    const bpmScale = timing.bpm / Math.max(1, referenceBpm);
    return basePixelsPerMs * bpmScale * timing.svMultiplier;
};

const getManiaScrollOffset = (startTime, endTime, playfieldHeight) => {
    ...
    for (const point of timingPoints) {
        ...
        distance += getManiaPixelsPerMs(segmentStart, playfieldHeight) * (point.time - segmentStart);
        segmentStart = point.time;
    }

    distance += getManiaPixelsPerMs(segmentStart, playfieldHeight) * (toTime - segmentStart);
...
const renderManiaMode = ({
    ...
    const currentPixelsPerMs = Math.max(0.001, getManiaPixelsPerMs(currentTime, scrollTravelHeight));
    ...
    for (const object of objects) {
        ...
        const rawHeadY = receptorY - getManiaScrollOffset(currentTime, object.time, scrollTravelHeight) - (noteHeight / 2);
        ...
        if (shouldRenderHoldBody) {
            const tailY = receptorY - getManiaScrollOffset(currentTime, object.endTime, scrollTravelHeight) + (noteHeight / 2);
...
const getStandardTimingState = (time) => {
    const timingPoints = Array.isArray(state.mapData?.timingPoints) ? state.mapData.timingPoints : [];
    let beatLength = 60000 / 120;
    let svMultiplier = 1;
...
const buildStandardSliderTicks = (object) => {
    if (!object || object.kind !== 'slider') {
        return [];
    }
    ...
    const slides = Math.max(1, object.slides || 1);
    const spanDuration = totalDuration / slides;
    const beatLength = getStandardTimingState(object.time).beatLength;
    const tickInterval = sliderTickRate > 0 ? (beatLength / sliderTickRate) : spanDuration;
    const ticks = [];
...
const render = () => {
    ...
    const { ctx, width, height } = getCanvasContext(state.canvas);
    ...
    for (const object of visibleObjects) {
        ...
        if (object.kind === 'slider') {
            ...
            if (STANDARD_SNAKING_SLIDERS && timeUntil > 0) {
                const fadeInElapsedMs = preemptMs - timeUntil;
            ...
            if (pathPoints.length > 1) {
                ...
                const sliderTicks = buildStandardSliderTicks(object);
                ...
                for (const tick of sliderTicks) {
                    ...
                    const tickElapsed = state.currentTime - tick.time;
        ...
        const sliderBallVisible = object.kind === 'slider'
            && state.currentTime >= object.time
            && state.currentTime <= object.endTime;
        const renderPrimaryCircle = object.kind !== 'slider'
            || state.currentTime < object.time
            || sliderBallVisible;
        if (object.kind === 'slider' && STANDARD_SLIDER_END_CIRCLES && sliderTailCanvasPoint) {
...
export const closeMapPreview = () => {
    ...
    state.catchHyperDashHitEffects = [];
    state.catchTriggeredHitEffects = new Set();
    state.catchLastEffectTime = Number.NaN;
    state.catchRenderObjects = null;
    if (state.toggleIndicator) {
        state.toggleIndicator.classList.remove('is-visible', 'is-play', 'is-pause');
    }
    if (state.popup) {
